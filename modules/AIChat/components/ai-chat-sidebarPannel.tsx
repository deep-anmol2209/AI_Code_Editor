...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
  ...
  const getChatModePrompt = (mode: string, content: string) => {
    switch (mode) {
      ...
      case "fix":
        return `please help to fix this code, including bugs, errors and potential problems:\n\n**Problem** ${content}`;

      case "optimize":
        return `please analyze this code for performance and suggest improvements:\n\n**Code to optimize:** ${content}`
      default:
        return content
    }
  };
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
     ...
     const messageType= chatMode === "chat"
     ? "chat"
     : chatMode === "review"
     ? "code_review"
     : chatMode === "fix"
     ...
     try {
      ...
      setMessages((prev)=>[
        ...prev, {
          role: "assistant",
          content: "having trouble connecting right now. please check your internet connection and try again.",
          timestamp: new Date(),
          id: Date.now().toString()
        }
      ])
  ...
  const filteredMessages = messages.filter((msg) => {
    ...
    return msg.content.toLowerCase().includes(searchTerm.toLocaleLowerCase())
  })
