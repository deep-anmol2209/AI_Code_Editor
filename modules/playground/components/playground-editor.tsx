...
import { configureMonaco, getEditorLanguage, defaultEditorOptions } from '@/modules/playground/lib/editor-config';
import * as monaco from "monaco-editor"
import type { TemplateFile } from '@/modules/playground/utils/playground-utils';
...
const PlaygroundEditor = ({activeFile, content, onContentChange,
  suggestion,
  suggestionLoading,
 suggestionPosition,
 onAcceptSuggestion,
 onRejectSuggestion,
onTriggerSuggestion}: PlaygroundEditorProps) => {
  const editorRef= useRef<any>(null)
  const monacoRef= useRef<Monaco | null>(null)
  const inlineCompletionProviderRef= useRef<monaco.IDisposable | null> (null)
  const currentSuggestionRef= useRef<{text: string
  ...
  const generateSuggestionId= ()=> `suggestion-${Date.now()}-${Math.random()}`
    ...
    const createInlineCompletionProvider = useCallback(
      (monaco: Monaco) => {
        return {
          provideInlineCompletions: async ( position: any, context: any, token: any) => {
